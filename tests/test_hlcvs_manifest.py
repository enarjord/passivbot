...
def test_verify_hlcvs_manifest_rejects_unknown_file_entry(tmp_path):
    cache_dir = tmp_path / "hlcvs"
    coins = ["BTC"]
    hlcvs = np.ones((2, 1, 4), dtype=np.float64)
    timestamps = np.array([1735689600000, 1735689660000], dtype=np.int64)
    ...
    manifest["files"]["unexpected_extra"] = {
        "path": "../outside.json",
        "sha256": "not-verified",
    }
    write_hlcvs_manifest(cache_dir, manifest)

    with pytest.raises(HlcvsManifestError, match="unknown file entries"):
        verify_hlcvs_manifest(cache_dir)

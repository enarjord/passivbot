...
def test_install_runtime_overrides_sets_hsl_sample_minutes():
    client = FakeCCXTClient(_scenario(), quote="USDT")
    bot = type("Bot", (), {"cca": client})()
    _install_runtime_overrides(bot, {"sample_minutes_override": 1.5})
    assert bot.get_exchange_time() == client.now_ms
    assert bot._equity_hard_stop_sample_minutes() == pytest.approx(1.5)


def test_refresh_halted_runtime_forced_modes_keeps_red_latched_pside_in_panic():
    from passivbot import Passivbot

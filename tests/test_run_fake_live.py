...
@pytest.mark.asyncio
@pytest.mark.parametrize('case', ['unordered_nonflattening', 'realized_loss', 'stale_reset', 'pending_pnl', 'invalid_fill', 'invalid_cache', 'invalid_metadata'])
async def test_coherent_hsl_evidence_with_real_rust_and_fake_exchange(tmp_path, monkeypatch, case):
    ...
    async def exercise(bot):
        ...
        if case == 'stale_reset':
            ...
            bot._pnls_manager.cache.load_metadata()['covered_start_ms'] = 'invalid'
        ...
        if case == 'unordered_nonflattening':
            ...
            if case in {'pending_pnl', 'invalid_fill', 'invalid_cache', 'invalid_metadata'}:
                assert not await recovery.protect_unready_hsl(bot)
                health = bot._hsl_protection_health.scopes[Scope('coin', 'long', symbol)]

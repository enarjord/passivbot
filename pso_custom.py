...
def pso_multiprocess(
    ...
    def get_new_velocity_and_position(
        velocity, position, lbest_, gbest_
    ...
    velocities = (
        np.random.random(positions.shape) - 0.5
    ) * 0.0001  # init velocities to small random number
        ...
        if itr_counter >= iters:
            ...
            if workers[worker_cycler] is None:
                if pos_cycler not in working:
                    pos_hash = sha256(
                        str(positions[pos_cycler]).encode("utf-8")
                    ).hexdigest()
                    for _ in range(100):
                    ...
                    tested.add(pos_hash)
                    workers[worker_cycler] = (
                        pos_cycler,
                        pool.apply_async(reward_func, args=(positions[pos_cycler],)),
                    )
...
class PostProcessing:
    ...
    def process(self, result):
        ...
        score = -score
        best_score = (
            self.all_backtest_analyses[0][0] if self.all_backtest_analyses else 9e9
        )
...
class BacktestWrap:
    ...
    def rf(self, xs):
        ...
        for key in [
            ...
            "n_stop_loss_closes",
            "biggest_psize",
            "mean_hrs_between_fills",
            "mean_hrs_between_fills_long",
            "mean_hrs_between_fills_short",
            "max_hrs_no_fills_long",
            "max_hrs_no_fills_short",
    ...
    for config in await prep_config(args):
        try:
            ...
            if config["starting_configs"]:
                ...
                initial_positions = [
                    backtest_wrap.config_to_xs(cfg) for cfg in starting_configs
                ]
            else:
                initial_positions = []
            pso_multiprocess(

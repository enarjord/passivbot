...
Push a local directory to a VPS, extract it on the remote side, and clean up
the temporary archives:

    ./sync_tar.py push ./data vps3:/opt/backups --remote-extract

Pull a remote directory down to the current machine, extract it locally, and
clean up the temporary archives on both sides:

    ./sync_tar.py pull vps3:/opt/backups ./restored --extract

Manually extract an archive that was previously transferred:

    ./sync_tar.py extract ./backups/project_20250101.tar.gz ./restore_here

The tool honours host aliases defined in ~/.ssh/config, so you may use either
plain hostnames/IPs or aliases (e.g. ``vps3``) when specifying the remote.
"""
...
def _run(cmd: list[str], *, check: bool = True) -> subprocess.CompletedProcess:
    """Run a subprocess command with console logging."""
...
def _resolve_pull_destination(destination: str, remote_base: str) -> Path:
    ...
    # If destination looks like a file path (has a suffix) and matches the remote base name,
    # extract into its parent instead of creating a directory named after the file.

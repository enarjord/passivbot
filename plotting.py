...
def dump_plots(result: dict, fdf: pd.DataFrame, sdf: pd.DataFrame, df: pd.DataFrame):
    ...
    table.add_row(
        ["Biggest psize", round_dynamic(result["result"]["biggest_psize"], 6)]
    )
    table.add_row(
        [
    ...
    table.add_row(
        ["Max hours no fills", round_dynamic(result["result"]["hrs_stuck_max_long"], 6)]
    )
    ...
    if result["long"]["enabled"]:
        ...
        table.add_row(
            ["No. inital entries", len(longs[longs.type.str.contains("ientry")])]
        )
        table.add_row(["No. reentries", len(longs[longs.type.str.contains("rentry")])])
        table.add_row(
            ["No. normal closes", len(longs[longs.type.str.contains("nclose")])]
        )
        table.add_row(
            [
                "Mean hours stuck (long)",
                round_dynamic(result["result"]["hrs_stuck_avg_long"], 6),
            ]
        )
        table.add_row(
            [
                "Max hours stuck (long)",
                round_dynamic(result["result"]["hrs_stuck_max_long"], 6),
            ]
        )
    ...
    if result["short"]["enabled"]:
        ...
        table.add_row(
            ["No. inital entries", len(shorts[shorts.type.str.contains("ientry")])]
        )
        table.add_row(
            ["No. reentries", len(shorts[shorts.type.str.contains("rentry")])]
        )
        table.add_row(
            ["No. normal closes", len(shorts[shorts.type.str.contains("nclose")])]
        )
        table.add_row(
            [
                "Mean hours stuck (short)",
                round_dynamic(result["result"]["hrs_stuck_avg_short"], 6),
            ]
        )
        table.add_row(
            [
                "Max hours stuck (short)",
...
def plot_fills(
    df, fdf_, side: int = 0, bkr_thr=0.1, plot_whole_df: bool = False, title=""
):
    if fdf_.empty:
        return
    plt.clf()

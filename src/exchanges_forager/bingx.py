    ...
    def set_market_specific_settings(self):
        ...
        for symbol in self.markets_dict:
            ...
            self.min_costs[symbol] = (
                elm["limits"]["cost"]["min"] if elm["limits"]["cost"]["min"] else 2.0
            )
    ...
    async def execute_order(self, order: dict) -> dict:
        ...
        try:
            ...
            if '"code":101400' in str_e:
                ...
                return {}
            if (
                "Due to the large market fluctuations, in order to reduce the risk of liquidation, API orders are temporarily disabled"
                in str_e
            ):
                logging.info(
                    f"debug {order} Due to the large market fluctuations, in order to reduce the risk of liquidation, API orders are temporarily disabled"
                )
                return {}

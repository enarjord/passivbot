...
def _install_runtime_overrides(bot, scenario: dict) -> None:
    ...
    sample_minutes_override = scenario.get("sample_minutes_override")
    if sample_minutes_override is None:
        return
    sample_minutes = float(sample_minutes_override)
    if sample_minutes <= 0.0:
        raise ValueError("fake scenario sample_minutes_override must be > 0")
    bot._equity_hard_stop_sample_minutes = lambda: sample_minutes

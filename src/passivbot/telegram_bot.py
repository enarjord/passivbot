...
try:
    ...
    print(e)
    print(
        "Unable to import git module. This is probably a result of a missing git installation."
...
class Telegram:
    def __init__(self, config: dict, bot, loop):
        ...
        first_keyboard_buttons = [
            [
                KeyboardButton("/daily \U0001F4B8"),
                KeyboardButton("/open_orders \U0001F4CB"),
                KeyboardButton("/position \U0001F4CC"),
            ],
            [
                KeyboardButton("/closed_trades \U0000274E"),
                KeyboardButton("/show_config \U0001F4DD"),
            ],
            [
                KeyboardButton("/balance \U0001F4B3"),
                KeyboardButton("/help \U00002753"),
                KeyboardButton("/next \U000023E9"),
            ],
        ]
        second_keyboard_buttons = [
            [
                KeyboardButton("/reload_config \U0000267B"),
                KeyboardButton("/set_short \U0001F4C9"),
                KeyboardButton("/set_long \U0001F4C8"),
            ],
            [
                KeyboardButton("/transfer \U0001F3E6"),
                KeyboardButton("/set_profit_transfer \U0001F4DD"),
                KeyboardButton("/set_config \U0001F4C4"),
            ],
            [
                KeyboardButton("/previous \U000023EA"),
                KeyboardButton("/stop \U000026D4"),
                KeyboardButton("/next \U000023E9"),
            ],
        ]
        ...
        self._keyboards = [
            ReplyKeyboardMarkup(first_keyboard_buttons, resize_keyboard=True),
    ...
    def add_handlers(self, updater):
        ...
        dispatcher.add_handler(
            ConversationHandler(
                ...
                states={
                    1: [
                        MessageHandler(
                            Filters.regex("(graceful|freeze|shutdown|panic|manual|resume|cancel)"),
                            self._stop_mode_chosen,
                        )
                    ],
                    2: [
                        MessageHandler(
                            Filters.regex("(confirm|abort)"), self._verify_stop_confirmation
                        )
                    ],
                },
        ...
        dispatcher.add_handler(
            ConversationHandler(
                ...
                states={
                    1: [
                        MessageHandler(
                            Filters.regex("(confirm|abort)"), self._verify_short_confirmation
                        )
                    ]
                },
                fallbacks=[CommandHandler("cancel", self._abort)],
        ...
        dispatcher.add_handler(
            ConversationHandler(
                ...
                states={
                    1: [
                        MessageHandler(
                            Filters.regex("(confirm|abort)"), self._verify_long_confirmation
                        )
                    ]
                },
                fallbacks=[CommandHandler("cancel", self._abort)],
        ...
        dispatcher.add_handler(
            ConversationHandler(
                ...
                states={
                    ...
                    2: [
                        MessageHandler(
                            Filters.regex("(/[0-9\\.]*)|cancel"), self._transfer_amount_chosen
                        )
                    ],
                    3: [
                        MessageHandler(
                            Filters.regex("(confirm|abort)"), self._verify_transfer_confirmation
                        )
                    ],
                },
        ...
        dispatcher.add_handler(
            ConversationHandler(
                entry_points=[
                    CommandHandler("set_profit_transfer", self._begin_set_profit_transfer)
                ],
                states={
                    1: [
                        MessageHandler(
                            Filters.regex("([0-9]*|cancel)"), self._profit_transfer_chosen
                        )
                    ],
                    2: [
                        MessageHandler(
                            Filters.regex("(confirm|abort)"),
                            self._verify_profit_transfer_confirmation,
                        )
                    ],
                },
                fallbacks=[CommandHandler("cancel", self._abort)],
        ...
        dispatcher.add_handler(
            ConversationHandler(
                ...
                states={
                    ...
                    2: [
                        MessageHandler(
                            Filters.regex("([0-9\\.]|confirm|abort)"),
                            self._verify_open_confirmation,
                        )
                    ],
                },
                fallbacks=[CommandHandler("cancel", self._abort)],
    ...
    def _position_side_chosen(self, update=None, context=None) -> int:
        ...
        if self.force_open_type in ["long", "shrt"]:
            ...
            update.effective_message.reply_text(
                "Action aborted", reply_markup=self._keyboards[self._keyboard_idx]
            )
            return ConversationHandler.END
        return 2
    ...
    def _transfer_type_chosen(self, update=None, context=None) -> int:
        ...
        update.effective_message.reply_text(
            text=text, parse_mode=ParseMode.HTML, reply_markup=ReplyKeyboardMarkup([["cancel"]])
        )
        return 2
    ...
    def _verify_transfer_confirmation(self, update: Update, _: CallbackContext) -> int:
        ...
        async def _transfer_wallet(type_: str, amount: float):
            result = await self._bot.transfer(
                type_=self.transfer_type, amount=self.transfer_amount, asset="USDT"
            )
            if "code" in result:
                self.send_msg(f'{result["msg"]}')
            else:
    ...
    def _verify_long_confirmation(self, update: Update, _: CallbackContext) -> int:
        if update.message.text == "confirm":
            ...
            self.send_msg(
                f'Request for {"disabling" if self._bot.do_long is True else "enabling"} long was aborted'
            )
        else:
            update.message.reply_text(
                text=f"Something went wrong, either <pre>confirm</pre> or <pre>abort</pre> was expected, but {update.message.text} was sent",
    ...
    def _verify_stop_confirmation(self, update: Update, _: CallbackContext) -> int:
        if update.message.text == "confirm":
            ...
            self.stop_mode_requested = ""
            self.send_msg("Request for activating stop-mode was aborted")
        else:
            self.stop_mode_requested = ""
            update.message.reply_text(
                text=f"Something went wrong, either <pre>confirm</pre> or <pre>abort</pre> was expected, but {update.message.text} was sent",
    ...
    def _help(self, update=None, context=None):
        msg = (
            "<pre><b>The following commands are available:</b></pre>\n"
    ...
    def _open_orders(self, update=None, context=None):
        ...
        table_msg = order_table.get_string(
            sortby="Price",
            border=True,
            padding_width=1,
            junction_char=" ",
        ...
        msg = (
            f"Current price: {round_(self._bot.price, self._bot.price_step)}\n"
            f"<pre>{table_msg}</pre>"
        )
        self.send_msg(
            msg, refreshable=True, callback_path="update_open_orders", query=update.callback_query
        )
        ...
        if "long" in self._bot.position:
            ...
            closest_long_reentry_price = max(
                [
                    o["price"]
                    for o in self._bot.open_orders
                    if o["position_side"] == "long" and o["side"] == "buy"
                ]
                or [0]
            )
            closest_long_close_price = min(
                [
                    o["price"]
                    for o in self._bot.open_orders
                    if o["position_side"] == "long" and o["side"] == "sell"
                ]
                or [0]
            )
            ...
            position_table.add_row(
                [
                    "Cost/balance",
                    round_dynamic(float(long_position["pbr"]), 3),
                    round_dynamic(float(shrt_position["pbr"]), 3),
                ]
            )
            position_table.add_row(
                [
    ...
    def _balance(self, update=None, context=None):
        if bool(self._bot.position):
            ...
            async def _balance_async():
                ...
                quot_balance = list(
                    asset for asset in account["balances"] if asset["asset"] == self._bot.quot
                )
                quot_balance = (
                    {"free": 0.0, "locked": 0.0} if len(quot_balance) == 0 else quot_balance[0]
                )
                coin_balance = list(
                    asset for asset in account["balances"] if asset["asset"] == self._bot.coin
                )
                coin_balance = (
                    {"free": 0.0, "locked": 0.0} if len(coin_balance) == 0 else coin_balance[0]
                )

                msg = (
                    f'Futures balance {self._bot.margin_coin if hasattr(self._bot, "margin_coin") else ""}:\n'
        ...
        if self._bot.exchange == "binance":
            ...
            async def send_daily_async(n_days: int):
                ...
                idf, bdf = get_daily_from_income(
                    income,
                    self._bot.position["wallet_balance"],
                ...
                for i in range(len(bdf)):
                    table.add_row(
                        [
                            bdf.index[i][:10],
                            f"{round_dynamic(bdf.abs_income.iloc[i], 3)} ({bdf.gain.iloc[i] * 100:.2f}%)",
                        ]
                    )

                pct_sum = (
                    ((self._bot.position["wallet_balance"] / starting_balance - 1) * 100)
    ...
    def notify_entry_order_filled(
        self, position_side: str, qty: float, fee: float, price: float, total_size: float
    ):
        if "notify_entry_fill" not in self.config or self.config["notify_entry_fill"] is True:
            icon = "\U0001F535"
            try:
                self.send_msg(
                    f"<b>{icon} {self._bot.exchange.capitalize()} {self._bot.pair}</b> Opened {position_side}\n"
    ...
    def send_msg(
        ...
        try:
            if query:
                self._update_msg(
                    query=query, msg=msg, callback_path=callback_path, refreshable=refreshable
                )
                return
    ...
    def _update_msg(
        self, query: CallbackQuery, msg: str, callback_path: str = "", refreshable: bool = False
    ) -> None:
        if refreshable:
            reply_markup = InlineKeyboardMarkup(
                [

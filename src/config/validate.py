...
def validate_config(config: dict, *, raw_optimize=None, verbose: bool = True, tracker=None) -> None:
    ...
    authoritative_mode = str(config["live"].get("authoritative_refresh_mode", "staged")).lower()
    if authoritative_mode not in {"legacy", "staged"}:
        raise ValueError(
            "config.live.authoritative_refresh_mode must be one of: legacy, staged"
        )
    ticker_strategy = str(config["live"].get("market_snapshot_ticker_strategy", "auto")).lower()
    ...
    try:
        ...
        raise TypeError("config.live.forager_score_hysteresis_pct must be numeric") from exc
    if not math.isfinite(hysteresis_pct) or hysteresis_pct < 0.0:
        raise ValueError("config.live.forager_score_hysteresis_pct must be finite and >= 0.0")

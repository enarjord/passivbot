...
class PnlsLookback:
    ...
    def to_window_ms(self, *, minimum_ms: int = 1) -> int | None:
        if self.is_all:
            return None
        return max(int(minimum_ms), int(round(self.days * 86_400_000.0)))

    def to_start_ms(self, now_ms: int, *, minimum_ms: int = 1) -> int | None:

...
- Detailed per-symbol min-effective-cost entry blocks now log at INFO, while the aggregate blocked-entry summary remains WARNING.
...
- Live execution now requires one coherent fresh snapshot of balance, positions, open orders, and fill events before placing or cancelling orders. If authoritative exchange state changes during a refresh cohort, Passivbot skips that execution cycle and waits for a clean follow-up cohort before acting.
- Hyperliquid now auto-detects and logs the account abstraction mode (`disabled` vs `unifiedAccount`) at startup/hourly market refresh and supports both balance payload contracts automatically. Added a read-only `passivbot tool hyperliquid-abstraction-probe` helper to inspect the current abstraction state explicitly.
- Hyperliquid staged live refresh now relies on one coherent positions+balance account snapshot without the earlier exchange-specific HIP-3 reserve-reconciliation layer. Combined with the staged execution barrier, this removes the old `REST`/`REST+open_orders` balance oscillation path and simplifies Hyperliquid live state handling.
- Hyperliquid now defaults to the staged authoritative refresh path unless the config explicitly opts back into `legacy`. Non-unified HIP-3 configs fail earlier during startup, before the first live-state refresh.
- Hyperliquid non-unified (`dexAbstraction`) accounts are now limited to vanilla perps in Passivbot. If any HIP-3/non-standard perp symbol appears in effective `approved_coins` or live exchange state, the bot now hard-fails with an upgrade-to-`unifiedAccount` message instead of trying to continue on mixed account state.
- Hyperliquid `portfolioMargin` accounts are now treated as unified-compatible for HIP-3/non-standard perps, and Hyperliquid balance parsing tolerates non-unified payloads without `assetPositions`.
- Live logs are now quieter by default: routine market-filter inventory chatter and small order-plan summaries were demoted to `DEBUG`, while larger rebalance waves and slower staged refresh diagnostics remain at `INFO`.

...
### Fixed
- **Optimizer multiprocessing now works under the unified CLI on spawn-based platforms** - `passivbot optimize ...` no longer fails with pickling errors for worker bootstrap/runtime objects such as the SIGINT initializer, evaluator objects, or DEAP fitness classes when launched through the unified CLI on macOS/Python spawn multiprocessing.

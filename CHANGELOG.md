...
- Hyperliquid live sizing now compensates for HIP-3 stock-perp entry-order margin that Hyperliquid subtracts from `fetch_balance()` even in cross mode. Resting non-reduce-only HIP-3 orders no longer make the bot misread its own reserved margin as equity loss and churn order sizes in cancel/replace loops.

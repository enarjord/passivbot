...
- Hardened credential sanitization for multi-word passphrases, exact exchange auth keys, and
  ...
  credential-alias CLI flags, prefixed environment credentials, text and structured auth/API-sign
  aliases, prefixed auth/sign/passphrase aliases, and explicit slash-form credential fields are also
  redacted.

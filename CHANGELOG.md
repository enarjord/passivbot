...
### Fixed
- **Forager zero-weight optimizer corner case** - `forager_score_weights` are now treated as relative weights and normalized before use. All-zero weights are interpreted as volume-only ranking instead of failing evaluation, so optimizer searches over `forager_score_weights_*` bounds no longer crash when a candidate lands on `0/0/0`.

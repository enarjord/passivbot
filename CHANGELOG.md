...
- **Hyperliquid HIP-3 position and symbol normalization** - Hyperliquid now merges base + `dex=xyz` clearinghouse position payloads and normalizes aliases (`xyz:*`, `XYZ:*`) to whatever key actually exists in `markets_dict` (including `XYZ-*`), preventing stock-perp position misses and symbol mismatches.

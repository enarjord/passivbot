...
- Backtests now inherit `market_orders_allowed`, `market_order_near_touch_threshold`, and `pnls_max_lookback_days` from `config.live` by default while still allowing explicit `config.backtest` overrides for those fields. When a backtest override differs from live, config loading logs that divergence and backtest startup logs the effective value source.
- Pre-v7.9 config migration now inserts `backtest.pnls_max_lookback_days = 0.0` for legacy backtest compatibility while preserving the existing `live.pnls_max_lookback_days` value.
- Pre-v7.9 config migration now also inserts `backtest.market_orders_allowed = false` when a legacy config enabled `live.market_orders_allowed` but did not explicitly override the backtest value, preventing surprise backtest changes after upgrading while leaving live behavior unchanged.
...
- Added optimizer override `mirror_short_from_long`, so optimization can keep `bot.short` mirrored to `bot.long` for the active strategy while still using the existing `optimize.enable_overrides` mechanism.
- `ema_anchor` now supports volatility-weighted spread widening with new strategy params `offset_volatility_ema_span_minutes`, `offset_volatility_1m_weight`, `entry_volatility_ema_span_hours`, and `offset_volatility_1h_weight`. The strategy widens quotes symmetrically around the EMA band while keeping inventory skew separate.
- Backtest artifacts now include `dataset.json` with the exact HLCV cache directory, absolute cache filepaths, and coin-index mapping used for the run. Added notebook-facing `ema_anchor_analysis` helpers for neutral bands, inventory-aware bands, and direct artifact loading so `fills.csv` / `balance_and_equity.csv.gz` can be analyzed against the exact cached candle stream that produced the backtest.
...
- `trailing_grid` grid closes can now anchor to either `position_price` or the side-relative EMA band via `bot.<side>.strategy.trailing_grid.grid_close_price_anchor`. Longs accept `ema_band` / `ema_band_upper`, shorts accept `ema_band` / `ema_band_lower`, and trailing closes remain anchored to position price.

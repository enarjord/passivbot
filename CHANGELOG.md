...
- Added shared `bot.<side>.risk.entry_cooldown_minutes`, which rate-limits new position-adding orders per side across strategies using stateless fill reconstruction and minute-aligned live/backtest cooldown timing. `ema_anchor` also adds `entry_double_down_factor`, which scales same-side entry size continuously with inventory bias.

...
### Added
- **Fill events doctor tool** - Added `src/tools/fill_events_doctor.py` to audit cached fill events and optionally auto-repair known Bybit duplicate-fill anomalies by refetching from exchange. Startup doctor runs are now supported via `PASSIVBOT_FILL_EVENTS_DOCTOR` (`check` or `repair`).

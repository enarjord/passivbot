...
- Hyperliquid staged balance reconciliation no longer feeds bot-managed resting-order reserve back into published balance on non-unified accounts. Live balance now restores only live HIP-3 position margin in that mode, which avoids self-induced balance and order-replacement churn when Passivbot cancels and recreates resting entry orders.

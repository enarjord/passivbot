...
### Added
- **Monitor publisher groundwork** - Live configs now support a new `monitor.*` section, and the bot can publish a disk-backed monitor root with a manifest, atomic `state.latest.json` snapshots, structured NDJSON events for lifecycle, balance, positions, orders, fills, modes, HSL, and structured bot/exchange errors, plus `history/` streams for normalized fills, throttled price ticks, and completed 1m/1h candles. The current snapshot now also includes `market`, `forager`, `unstuck`, and `recent` sections for richer read-only bot state inspection.
- **Read-only monitor relay server** - Added `src/tools/monitor_relay.py`, a separate monitor relay process that reads the disk-backed monitor root and serves `GET /health`, `GET /snapshot`, and `GET /ws` for snapshot bootstrap plus live-forward event/history streaming to TUI, browser, mobile, or remote consumers.
...
- **Browser dashboard polish pass** - The first web dashboard now contains long event/order/trailing content inside its cards more cleanly, uses compact summaries instead of raw payload dumps in narrow panels, highlights the current focus symbol visually, and lets you click symbol-bearing rows in positions, ticks, trailing, events, and orders to change focus quickly.
- **Minimal monitor TUI reader** - Added `src/tools/monitor_tui.py`, a first terminal dashboard that consumes the relay. It refreshes `/snapshot` for current-state panels, consumes `/ws` for recent events and price ticks, and renders a small in-place terminal view for account, health, HSL, active positions/orders, and recent activity.
- **Monitor dev launcher + embedded log tail** - Added `src/tools/monitor_dev.py` for local monitor iteration. It reuses or starts the relay automatically, selects the latest bot log by default, and shows recent bot log lines inside the TUI so relay/TUI/log watching can be done from one command.
- **Focused-symbol monitor TUI panels** - The monitor TUI now supports `--focus-symbol` and shows a denser live view with focused-symbol market/position detail plus recent order activity from the current snapshot, making it more useful during live bot supervision without changing the relay protocol.
- **Runtime monitor TUI command prompt** - The TUI now has a bottom command line for in-session actions. Current commands include `focus BTC`, `focus next`, `focus prev`, `focus auto`, `help`, `quit`, and `exit`, with symbol alias matching so a base coin like `BTC` is enough unless multiple quotes exist.
- **Detailed monitor position metrics** - Monitor snapshots now publish per-side position exposure metrics including WE/WEL/WELe/TWEL ratios, cached last price, price-action distance, and uPnL, and the TUI positions panel now renders those fields in a log-style operator view.
- **Trailing monitor visibility** - Monitor snapshots now publish a `trailing` section with observability-only next trailing entry/close state, threshold/retracement trigger levels, met-status flags, and trailing extrema snapshots. The monitor TUI now renders a dedicated `Trailing` box so operators can see which positions are currently on trailing logic and how close they are to triggering.
...
- **Less flickery monitor TUI redraw + screen controls** - The TUI now repaints only when the rendered frame changes, no longer does a full-screen clear on every timer tick, and now applies row-diff ANSI updates instead of rewriting the entire frame on each change. It also uses a boxed multi-panel layout on wider terminals, and the command prompt now supports `pause`, `resume`, and `dump`, so operators can freeze the data panels for copying while keeping the command footer live or save the current screen to `tmp/monitor_tui_dump_*.txt`.
- **Richer monitor operator panels** - The monitor snapshot now carries current EMA-band hints in `market`, next-in-line forager selection hints in `forager`, and unstuck target/EMA-trigger hints in `unstuck`. The TUI now uses that data for dedicated `Forager` and `Unstuck` boxes, shows EMA bands in the price-ticks panel, removes per-row TWEL duplication from position rows, and summarizes total TWE once at the positions-box level.
...
- **HSL restart metrics by side** - Backtest HSL analysis now also exports `hard_stop_restarts_per_year_long` and `hard_stop_restarts_per_year_short`, while `hard_stop_restarts_per_year` remains the unchanged aggregate annualized restart rate across both sides.
- **EMA-smoothed HSL drawdown metrics** - Backtest analysis now also exports `drawdown_worst_ema_hsl` / `drawdown_worst_mean_1pct_ema_hsl` plus the side-specific `_long` / `_short` variants. The side metrics use each `pside`'s configured `hsl_ema_span_minutes`, and the shared metrics are reported conservatively as `max(long, short)`.
- **HSL drawdown plot now uses true runtime HSL state** - The `hard_stop_drawdown` backtest figure now plots the runtime `drawdown_ema` and `drawdown_score` series emitted by Rust instead of reconstructing them only in Python, and overlays RED-entry, halt/cooldown, terminal-halt, and restart events directly on the per-side charts.
...
- **Live margin-mode preference for CCXT exchanges** - Added `live.margin_mode_preference` with `auto`, `auto_cross`, `auto_isolated`, `cross`, and `isolated` behaviors. New-entry approval now respects exchange margin capabilities, while existing positions/orders remain manageable on their actual live margin mode after restart.

### Fixed
- **Live operator silence watchdog** - Live bots now emit an INFO-level watchdog message when no logs have been emitted for a configurable interval (`logging.silence_watchdog_seconds`, default `60`). The message includes the current startup/runtime stage so long fill-history syncs, HSL history replay, and other slow operations are less likely to look like a freeze. Open-ended fill refreshes also now emit progress logs when they stay quiet for multiple seconds.
- **Monitor relay/TUI attach and readability polish** - New TUI clients now hydrate recent events and price ticks immediately when attaching to an already-running bot because the relay replays a small recent tail from the current files on websocket connect. The terminal dashboard also now avoids right-edge auto-wrap artifacts, adds modest ANSI color accents, shows labeled EMA-band bounds in the price-ticks panel, surfaces richer forager ranking/next-entry context, collapses repeated `account.balance` websocket spam, and keeps command feedback such as `help`/`focus`/`dump` visible in the command box.
- **Monitor TUI panel cleanup** - The terminal dashboard now skips disabled-empty short sections in the `Forager` and `Unstuck` boxes, adds an explicit `ranking:` label in the forager panel, and makes the `help` command status line more visually distinct.
...
- **`vpssync.sh pull` wildcard handling** - Pulling remote paths with quoted wildcard patterns such as `logs/20260318*` now archives remote matches correctly instead of passing the pattern literally to `tar`, uses a sane local archive name without `*`, and extracts into the parent directory rather than creating a local path literally named after the glob.
- **Hyperliquid HIP-3 restart visibility** - Live Hyperliquid state refresh now includes dex-scoped HIP-3 position/open-order discovery for tracked and approved symbols on cold restart, preventing existing isolated HIP-3 positions from becoming invisible and triggering duplicate fresh entries after a restart.
- **Cross/isolated restart handling on supported exchanges** - Bybit, Binance, Bitget, KuCoin, and OKX now honor `live.margin_mode_preference` consistently for symbol config updates and restart-time state management, preserving the actual live margin mode of existing positions/orders instead of forcing cross mode. `defx` and `paradex` are temporarily disabled in this dev branch pending resumed support work.
- **Hyperliquid HIP-3 isolated trading disabled for now** - Passivbot now treats Hyperliquid HIP-3 as cross-only for live trading until isolated-margin support is properly designed. Cross-capable HIP-3 markets are forced to cross for new entries, isolated-only HIP-3 markets are skipped with warnings, and existing isolated HIP-3 positions or open orders cause startup to fail loudly instead of running in a risky partial-support mode.

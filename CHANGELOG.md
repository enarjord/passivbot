...
- **Dynamic per-symbol margin-mode preference** - Passivbot now applies a generic `live.margin_mode_preference` policy per symbol using exchange metadata. Supported values are `auto`/`auto_cross`/`auto_cross_preferred`, `auto_isolated`/`auto_isolated_preferred`, `cross`, and `isolated`. Symbols incompatible with strict `cross` or `isolated` preference are loudly disabled for new entries instead of crashing the bot, while existing positions remain manageable.

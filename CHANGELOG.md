...
### Changed
- **Config loading now uses a canonical staged pipeline** - Defaults now come only from in-code schema, `load_config()` / `format_config()` normalize to canonical user-facing keys without leaking runtime `filter_*` aliases, runtime aliasing moved into explicit compilation helpers, and the default example config path is now `configs/examples/template.json` instead of `configs/template.json`.

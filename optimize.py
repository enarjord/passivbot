    ...
    for k, v in unpack_config(start_config).items():
        if k in config:
            if (
                ...
                clean_start[k] = min(
                    max(v, config["ranges"][k][0]), config["ranges"][k][1]
                )
    return clean_start
...
def iter_slices(data, sliding_window_days: float, max_span: int):
    ...
    for x in np.linspace(
        len(data) - samples_per_window, max_span_ito_n_samples, n_windows
    ):
        start_i = max(0, int((x - max_span_ito_n_samples)))
        end_i = min(
            len(data), int(round(start_i + samples_per_window + max_span_ito_n_samples))
        )
...
def single_sliding_window_run(config, data, do_print=True) -> (float, [dict]):
    ...
    for z, data_slice in enumerate(
        ...
        analysis["score"], do_break, line = objective_function(
            analysis, config, metric=metric
        )
...
def backtest_tune(
    ...
    ray.init(
        ...
        object_store_memory=memory if memory > 4000000000 else None,
    ...
    backtest_wrap = tune.with_parameters(
        ...
        data=data,
        do_print=(
            config["print_slice_progress"]
            if "print_slice_progress" in config
            else False
        ),
    )
    analysis = tune.run(
...
def save_results(analysis, config):
    ...
    df.rename(
        columns={column: column.replace("config.", "") for column in df.columns},
        inplace=True,
    )

...
def backtest_single_wrap(config_: dict):
    ...
    try:
        ...
        print(
            f"backtested {config['symbol']: <12} pa closeness long {pa_closeness_long:.6f} "
...
def backtest_multi_wrap(config: dict, pool):
    ...
    for s in sorted(config["symbols"]):
        tasks[s] = pool.apply_async(
            backtest_single_wrap, args=({**config, **{"symbol": s}},)
        )
    while True:
    ...
    if config["side"] == "long":
        score = adg_mean * min(
            1.0, config["maximum_pa_closeness_mean_long"] / pa_closeness_long_mean
        )
    elif config["side"] == "short":
        score = adg_mean * min(
            1.0, config["maximum_pa_closeness_mean_short"] / pa_closeness_short_mean
        )
    elif config["side"] == "both":
        score = adg_mean * min(
            1.0,
            (
                config["maximum_pa_closeness_mean_long"] / pa_closeness_long_mean
                + config["maximum_pa_closeness_mean_short"] / pa_closeness_short_mean
            )
            / 2,
        )

    print(
        f"pa closeness long {pa_closeness_long_mean:.6f} pa closeness short {pa_closeness_short_mean:.6f} adg {adg_mean:.6f} score {score:8f}"
    )
...
def harmony_search(
    ...
    hm = numpyize(
        [
            [
                np.random.uniform(bounds[0][i], bounds[1][i])
    ...
    for itr in range(iters):
        ...
        print("best harmony")
        print(
            round_values(denumpyize(hm[hm_evals.argmin()]), 5), f"{hm_evals.min():.8f}"
        )
...
class FuncWrap:
    def __init__(self, pool, base_config):
        ...
        self.results_fname = make_get_filepath(
            f"tmp/harmony_search_results_{self.test_symbol}_{self.now_date}.txt"
        )
        self.best_conf_fname = (
            f"tmp/harmony_search_best_config_{self.test_symbol}_{self.now_date}.json"
        )
    ...
    def func(self, xs):
        ...
        with open(self.results_fname, "a") as f:
            f.write(
                json.dumps(
                    {"config": candidate_to_live_config(config), "results": results}
                )
                + "\n"
            )
        return score

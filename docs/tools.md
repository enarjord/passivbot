...
- `passivbot tool live-incident-bundle` writes a local `.tar.gz` evidence
  ...
  performance sections as compact filter provenance. Its planned smoke command
  uses the restart planner's bounded scan defaults rather than the incident
  bundle's event/log scan settings.

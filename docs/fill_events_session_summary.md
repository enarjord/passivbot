...
python3 src/tools/fill_events_dash.py \
  ...
  --config configs/examples/default_trailing_martingale_long_npos4.json \
  --lookback-days 60 \
  --port 8050 \
  --log-level info
```
...
- **Location**: `configs/examples/default_trailing_martingale_long_npos4.json`, `src/config/schema.py`

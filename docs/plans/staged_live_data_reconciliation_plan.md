...
- [x] Ensure open-tail EMA projection is bounded by the configured staleness threshold and never
  persisted as real exchange candles or normal EMA cache entries.
- [x] Resolve known live/backtest EMA parity risk for open-ended tail gaps. Staged live now uses
  provisional in-memory EMA projection for close, quote-volume, and log-range as if missing tail
  minutes were no-trade candles. Projection is bounded by
  `live.max_active_candle_tail_gap_minutes`, stateless per read, and discarded when real candles
  arrive.
- [ ] Add tests for open-ended tail gaps:
  ...
  - [x] Provisional in-memory EMA tail projection for derived EMAs.
  - [x] Provisional in-memory EMA tail projection parity against bounded no-trade gaps.
  - [x] Bounded threshold halt for affected symbol only.
  - [ ] Ticker-liveness diagnostics.
  - [x] Recovery/recompute when late real candles replace previously projected timestamps.
  - [x] No suppression of active position management without visible projection policy state.

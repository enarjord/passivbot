...
| 2026-07-16 | #3 Live restart/smoke automation | active `codex/smoke-process-only-sampling` slice after PR #1283 / `9bcf6c24` | Separates bounded process/supervisor validation into a dedicated local-only command after the full smoke command was rejected as exchange-adjacent; records the preserved rejection and unchanged VPS5 pane processes | Merge and run the process-only VPS5 check; safe pull/stop/start orchestration remains open |

...
- **gap_tolerance_ohlcvs_minutes**: Maximum hole size that can be accepted immediately in prepared OHLCV data. Larger or persistent gaps are repaired from local v2 data, legacy shards, and targeted remote fetches before being recorded as explicit coverage metadata. Verified exchange-side late starts, early ends, and internal gaps do not by themselves abort a run, but local corruption, malformed candles, missing BTC benchmark data, or no tradable candles still fail loudly.

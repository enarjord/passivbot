...
- **max_active_candle_tail_gap_minutes**: Maximum open-ended 1m candle tail gap tolerated for active symbols before staged live planning blocks that symbol's trading-critical candle surface. Default is `10`. Within this bound, Passivbot may continue using the last real candle/EMA state while retrying candle fetches; bounded historical gaps still need real candles before and after before synthetic no-trade candles are replayed. This bounded carry-forward is a known live/backtest parity compromise: backtests can replay bounded no-trade gaps as synthetic zero-candles immediately because they know future candles, while live cannot prove whether the newest missing candle is exchange lag or no-trade data until later.
...
- **fixed_params**: List of dotted config-path selectors to freeze at the current config value for the whole run. Selectors match path segments by prefix, not substring. The leading `bot.` may be omitted for side-local paths, so `long.strategy` freezes `bot.long.strategy.<active_strategy>.*` and leaves `bot.long.risk`, `bot.long.forager`, and `bot.long.unstuck` tunable. A `*` path segment is a one-segment wildcard. `--fine_tune_params` uses the same selector contract for the inverse operation.

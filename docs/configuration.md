...
- **exchanges**: Exchanges from which to fetch 1m OHLCV data for backtesting and optimizing. Supported exchanges include `binance`, `bybit`, `gateio`, and `bitget`. The current default profile uses `['binance', 'bybit']`.
  ...
  **Hyperliquid note:** Hyperliquid's public candle endpoint only serves the most recent 5000 candles. For older full-day gaps from 2025-03-22 onward, Passivbot can derive 1m OHLCV from official requester-pays S3 raw fills/trades (`hl-mainnet-node-data`) and cache them under the normal OHLCV cache, provided local AWS credentials and the `lz4` CLI are available.

...
- **risk_input_max_attempts**: Failed-attempt limit for current-balance and HSL-history/evidence recovery. Default `10`, integer `>= 1`; the first failure counts and changing reasons does not renew the count. With HSL enabled, the limit escalates diagnostics to errors while capped retries and protective exits continue; it never terminates HSL protection. Unavailable HSL evaluation conservatively closes exposed HSL-enabled scopes through the configured panic path, potentially before RED, and blocks ordinary trading until fresh flat/order confirmation and input recovery. Proven cooldown/manual ownership remains unchanged. With HSL disabled, exhaustion stops without automatic restart. First and limit-reaching failures include bounded tracebacks.

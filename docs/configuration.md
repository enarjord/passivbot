...
- **order_replacement_churn_gate_tracking_tolerance_pct**: Wider price/quantity tolerance used only to associate historical Rust ideals as churn evidence. Default `0.002` (0.2%). It never widens actual-order reconciliation.
  - Configs containing the retired `initial_entry_exec_max_market_dist_pct` migrate automatically. A positive value becomes `order_replacement_churn_gate_market_dist_pct`, with other missing churn-gate settings supplied from the canonical defaults. A non-positive value, which disabled the retired gate, becomes `order_replacement_churn_gate_activation_count = 0`. Conflicting explicit old and new settings fail with an actionable error instead of choosing silently.

...
- **allow_simultaneous_grid_entries**: Allows multiple position-adding entry orders to be staged at once when the strategy entry path does not require retracement. Default `true`, preserving normal grid-ladder staging. Set `false` to keep only the first position-adding entry order on the book. Strategies with entry retracement enabled still stage one add order at a time so threshold/retracement state remains sequential.
- **entry_cooldown_minutes**: Time-based cooldown after the last position-increasing fill for that coin+pside. Values below `1.0` do not enforce a time cooldown; `1.0` blocks new adds through the next whole-minute boundary after the fill, and larger values block adds until that many rounded-up minutes have elapsed. This does not control simultaneous grid staging; use `allow_simultaneous_grid_entries` for that.

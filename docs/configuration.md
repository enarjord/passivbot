...
- **enable_forager_ws_candles**: Persists proven-final public 1m WebSocket candles for flat forager candidates when the exchange supports CCXT Pro `watchOHLCV`. REST remains the complete fallback for startup basis, gaps, reconnect recovery, prolonged silence, and periodic integrity audits. WebSocket silence never proves a no-trade candle, and unstable streams cool down while REST maintenance continues. Default is `true`; set `false` to use REST-only candidate maintenance. A zero `max_ohlcv_fetches_per_minute` disables this network path too.

...
- **enable_forager_ws_candles**: Uses finalized public 1m WebSocket candles as a non-persistent tail overlay for flat forager candidates when the exchange supports CCXT Pro `watchOHLCV`. REST remains authoritative for startup, gaps, corrections, persistence, and periodic overlap audits. WebSocket silence never proves a no-trade candle. Default is `true`; set `false` to use REST-only candidate maintenance. A zero `max_ohlcv_fetches_per_minute` disables this network path too.
...
- **forager_ws_candle_rest_audit_minutes**: Maximum time a healthy WebSocket-assisted flat-candidate 1m tail may defer its next authoritative REST overlap refresh. Default is `30`; accepted values are greater than `0` and at most `60`. Missing basis, internal gaps, WebSocket silence, reconnect gaps, and active position/open-order surfaces continue to use the existing REST readiness and refresh rules.

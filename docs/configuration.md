...
- **order_replacement_churn_gate_stability_minutes**: Minimum observed duration needed to prove sustained monotonic price or quantity drift, or repeated exclusive reappearance of the same semantic order cohort. Exclusive switching requires at least three appearances of the current cohort with unchanged ladder cardinality; its duration is fixed from the preceding appearance to the current run's start, while first appearance and first reappearance remain fail-open. A newest contiguous tight-match duration of the same length clears older evidence and its completed stable prefix becomes the symbol's switching-history boundary. Default `2.0` minutes.

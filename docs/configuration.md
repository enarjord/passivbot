...
- **allow_simultaneous_grid_entries**: Allows multiple position-adding entry orders to be staged at once only when the strategy entry path does not require retracement and `entry_cooldown_minutes = 0.0`. Default `true`, preserving normal grid-ladder staging for zero-cooldown non-retracement configs. Set `false` to keep only the first position-adding entry order on the book. Strategies with entry retracement enabled still stage one add order at a time so threshold/retracement state remains sequential.
- **entry_cooldown_minutes**: Time-based cooldown after the last position-increasing fill for that coin+pside. `0.0` disables the cooldown and may permit simultaneous entry ladders when `allow_simultaneous_grid_entries = true` and entry retracement is disabled. Any positive value enforces a cooldown of that many minutes and limits staged position-adding entries to one order, including fractional values such as `0.05` for roughly three seconds.

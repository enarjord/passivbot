...
- **risk_input_max_attempts**: Maximum failed attempts to establish valid current balances, required HSL replay balances, and HSL episode evidence per recovery episode. Default `10`, integer `>= 1`; the first failure counts. Retries stay in the same process with increasing delays. Changing failure reasons does not renew the budget. Successful recovery resets it; exhaustion logs an error and stops without full-bot auto-restart or spending `max_n_restarts_per_day`. Every attempt logs its count, limit, safe inputs, and next delay; first and final failures include bounded tracebacks. `1` stops immediately; there is no unlimited setting.

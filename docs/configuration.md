...
- **config_version**: Top-level schema version string for the config file. Canonical V8 configs must use `v8.3.0`. V8 is a breaking config schema and does not automatically convert v7 or pre-v8 configs; start from a V8 example config and port settings manually.
...
- **order_replacement_churn_gate_stability_minutes**: Minimum observed duration needed to prove sustained monotonic price or quantity drift, or repeated exclusive reappearance of the same semantic order cohort. Exclusive switching requires at least three appearances of the current cohort while every recurring cohort retains its ladder cardinality; its duration is fixed from the preceding appearance to the current run's start, while first appearance and first reappearance remain fail-open. A newest contiguous tight-match duration of the same length clears older evidence and its completed stable prefix becomes the symbol's observable switching-history boundary. Default `2.0` minutes.
...
- **risk_input_max_attempts**: Maximum failed attempts to establish valid current and required HSL replay balances per recovery episode. Default `10`, integer `>= 1`; the first failure counts. Retries stay in the same process with increasing delays. Changing failure reasons does not renew the budget. Successful recovery resets it; exhaustion logs an error and stops without full-bot auto-restart or spending `max_n_restarts_per_day`. Every attempt logs its count, limit, safe inputs, and next delay; first and final failures include bounded tracebacks. `1` stops immediately; there is no unlimited setting.

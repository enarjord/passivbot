...
from njit_funcs import round_, calc_long_pnl, calc_short_pnl, calc_diff
...
class Telegram:
    def __init__(self, config: dict, bot, loop):
        ...
        third_keyboard_buttons = [
            [
                KeyboardButton("/previous \U000023EA"),
                KeyboardButton("/force_open \U0001F3A2"),
            ]
        ]
    ...
    def add_handlers(self, updater):
        ...
        dispatcher.add_handler(
            CallbackQueryHandler(self._closed_trades, pattern="closed_trades")
        )
        dispatcher.add_handler(
            ...
            ConversationHandler(
                ...
                states={
                    1: [
                        MessageHandler(
                            Filters.regex(
                                "(graceful|freeze|shutdown|panic|manual|resume|cancel)"
                            ),
                    ...
                    2: [
                        MessageHandler(
                            ...
                            self._verify_stop_confirmation,
                        )
                    ],
                },
            ...
            ConversationHandler(
                ...
                states={
                    1: [
                        MessageHandler(
                            Filters.regex("(confirm|abort)"),
                            self._verify_short_confirmation,
                        )
                    ]
                },
                fallbacks=[CommandHandler("cancel", self._abort)],
            ...
            ConversationHandler(
                ...
                states={
                    1: [
                        MessageHandler(
                            Filters.regex("(confirm|abort)"),
                            self._verify_long_confirmation,
                        )
                    ]
                },
                fallbacks=[CommandHandler("cancel", self._abort)],
        ...
        dispatcher.add_handler(
            ConversationHandler(
                ...
                states={
                    ...
                    2: [
                        MessageHandler(
                            Filters.regex("(/[0-9\\.]*)|cancel"),
                            self._transfer_amount_chosen,
                        )
                    ],
                    3: [
                        MessageHandler(
                            Filters.regex("(confirm|abort)"),
                            self._verify_transfer_confirmation,
                        )
                    ],
                },
        ...
        dispatcher.add_handler(
            ConversationHandler(
                entry_points=[
                    CommandHandler(
                        "set_profit_transfer", self._begin_set_profit_transfer
                    )
                ],
                states={
                    1: [
    ...
    def _begin_transfer(self, update: Update, _: CallbackContext) -> int:
        ...
        buttons = [
            [
                InlineKeyboardButton(
                    "Spot -> USD-M Futures", callback_data="MAIN_UMFUTURE"
                ),
                InlineKeyboardButton(
                    "USD-M Futures -> Spot", callback_data="UMFUTURE_MAIN"
                ),
                InlineKeyboardButton("cancel", callback_data="cancel"),
            ]
        ]
    ...
    def _verify_transfer_confirmation(self, update: Update, _: CallbackContext) -> int:
        ...
        task = self.loop.create_task(
            _transfer_wallet(self.transfer_type, self.transfer_amount)
        )
        task.add_done_callback(
            lambda fut: True
        )  # ensures task is processed to prevent warning about not awaiting
    ...
    def _verify_set_short(self, update: Update, _: CallbackContext) -> int:
        reply_keyboard = [["confirm", "abort"]]
        update.message.reply_text(
            text=f'Shorting is currently <pre>{"enabled" if self._bot.do_short is True else "disabled"}</pre>.\n'
            f'You have chosen to <pre>{"disable" if self._bot.do_short is True else "enable"}</pre> shorting.\n'
    ...
    def _verify_short_confirmation(self, update: Update, _: CallbackContext) -> int:
        if update.message.text == "confirm":
            ...
            self.send_msg(
                f'Shorting is now <pre>{"enabled" if self._bot.do_short is True else "disabled"}</pre>.\n'
                "Please be aware that this change is NOT persisted between restarts."
            )
        elif update.message.text == "abort":
            self.send_msg(
                f'Request for {"disabling" if self._bot.do_short is True else "enabling"} shorting was aborted'
            )
        else:
            update.message.reply_text(
    ...
    def _verify_long_confirmation(self, update: Update, _: CallbackContext) -> int:
        if update.message.text == "confirm":
            ...
            self.send_msg(
                f'Long is now <pre>{"enabled" if self._bot.do_short is True else "disabled"}</pre>.\n'
                "Please be aware that this change is NOT persisted between restarts."
            )
        elif update.message.text == "abort":
            self.send_msg(
    ...
    def _profit_transfer_chosen(self, update: Update, _: CallbackContext) -> int:
        ...
        try:
            ...
            if (
                self.profit_transfer_pct_chosen < 0
                or self.profit_transfer_pct_chosen > 1
            ):
                self.send_msg(
    ...
    def _verify_profit_transfer_confirmation(
        self, update: Update, _: CallbackContext
    ) -> int:
        if update.message.text == "confirm":
            self._bot.set_config_value(
                "profit_trans_pct", self.profit_transfer_pct_chosen
            )
            self.send_msg(
    ...
    def _open_orders(self, update=None, context=None):
        ...
        self.send_msg(
            msg,
            refreshable=True,
            callback_path="update_open_orders",
            query=update.callback_query,
        )
        ...
        if bool(self._bot.position):
            ...
            async def _balance_async():
                ...
                quot_balance = list(
                    ...
                    if asset["asset"] == self._bot.quot
                )
                quot_balance = (
                    {"free": 0.0, "locked": 0.0}
                    if len(quot_balance) == 0
                    else quot_balance[0]
                )
                coin_balance = list(
                    asset
                    for asset in account["balances"]
                    if asset["asset"] == self._bot.coin
                )
                coin_balance = (
                    {"free": 0.0, "locked": 0.0}
    ...
    def _closed_trades(self, update=None, context=None):
        if self._bot.exchange == "binance":
            ...
            async def send_closed_trades_async():
                ...
                table = PrettyTable(
                    ["Date", "Pos.", "Price", f"Pnl {self._bot.margin_coin}"]
                )
    ...
    def _daily(self, update=None, context=None):
        if self._bot.exchange == "binance":
            ...
            async def send_daily_async(n_days: int):
                ...
                pct_sum = (
                    (
                        (self._bot.position["wallet_balance"] / starting_balance - 1)
                        * 100
                    )
                ...
                table.add_row(
                    [
                        "Total",
                        f"{round_dynamic(income_sum, 3)} ({round_(pct_sum, 0.01)}%)",
                    ]
                )
    ...
    def notify_entry_order_filled(
        ...
        if (
            "notify_entry_fill" not in self.config
            or self.config["notify_entry_fill"] is True
        ):
            icon = "\U0001F535"
            try:
                self.send_msg(
    ...
    def notify_close_order_filled(
        ...
        if (
            "notify_close_fill" not in self.config
            or self.config["notify_close_fill"] is True
        ):

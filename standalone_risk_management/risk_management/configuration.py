...
def _debug_to_logging_level(debug_level: int) -> int:
    """Translate the risk manager debug scale into logging module levels."""
...
def _resolve_external_logging_configurator() -> Optional[Callable[..., Any]]:
    """Return an optional ``logging_setup.configure_logging`` style callable if present."""

    return _cached_external_logging_configurator()


@lru_cache(maxsize=1)
def _cached_external_logging_configurator() -> Optional[Callable[..., Any]]:
    spec = importlib.util.find_spec("logging_setup")
    if spec is None:  # pragma: no cover - optional module missing in tests
        return None
...
def _configure_default_logging(debug_level: int = 1) -> bool:
    """Provision sensible logging defaults matching the realtime tooling."""

    root_logger = logging.getLogger()
    already_configured = bool(root_logger.handlers)

    if not already_configured:
        configurator = _resolve_external_logging_configurator()
